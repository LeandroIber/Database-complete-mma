...
Todos os scripts de construção e correção do banco estão em `scripts/` e `sql/`:

```bash
# 1. Extrair title fights canônicos da Wikipedia
python scripts/01_extract_titles.py

# 2. Aplicar correções no banco
python scripts/02_apply_corrections.py

# 3. Validar correções
python scripts/03_validate.py

# 4. Scrapear recordes oficiais (a partir de HTMLs salvos)
python scripts/scraper_ufc.py
```

Scripts SQL para criação e linkagem em `sql/`:

- `sql/create_records_tables.sql`
- `sql/link_records.sql`
...
- **Código** (scripts Python, SQL): MIT License
- **Dados** (banco DuckDB, CSVs): CC-BY-4.0 (atribuir o autor + fontes Sherdog/UFCStats/Wikipedia)
- **HTMLs originais**: copyright dos respectivos detentores, mantidos apenas para auditoria local
...
```
Leandro [Seu Sobrenome]. (2026). MMA Global Dataset: Lutas estruturadas de MMA 
com validação cruzada (1993-2026). GitHub.
https://github.com/[seu-usuario]/mma-global-dataset
```
...
- Issues: [github.com/seu-usuario/mma-global-dataset/issues](https://github.com/seu-usuario/mma-global-dataset/issues)
